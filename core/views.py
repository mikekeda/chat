...
def user_map(request):
    ...
    users = [
        {
            ...
            "profile": reverse("core:user", kwargs={"username": user.user__username}),
        }
        for user in users
    ]
    ...
    return render(
        request,
        "users_map.html",
        {"users": users, "google_map_api_key": settings.GOOGLE_MAP_API_KEY},
    )
...
class ProfileView(View, GetUserMixin):
    ...
    def get(self, request, username):
        ...
        threads = (
            ...
            .values_list("id", "name", "last_message", named=True)
        )
...
class ThreadView(View, GetUserMixin):
    ...
    def get(self, request, username=None, thread_id=None):
        ...
        if username:
            ...
            thread = (
                Thread.objects.annotate(count=Count("users"))
        ...
        messages = (
            Message.objects.select_related("user", "user__profile")
