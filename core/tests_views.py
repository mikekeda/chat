...
class ChatViewTest(TestCase):
    ...
    def test_views_chat(self):
        ...
        # Check the database.
        chat = Thread.objects.filter(users__username="testuser").filter(
            users__username="testuser2"
        )
        ...
        resp = self.client.get(reverse("core:thread", kwargs={"thread_id": chat[0].pk}))
        self.assertEqual(resp.status_code, 200)
        self.assertTemplateUsed(resp, "thread.html")

        # Tread with id 404 doesn't exists.
        resp = self.client.get(reverse("core:thread", kwargs={"thread_id": "404"}))
